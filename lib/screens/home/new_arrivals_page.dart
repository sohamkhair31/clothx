...
const _mockBrands = ["NV'S", "NV'S Icon", "NV'S Sport"];
...
List<NVProduct> _mockProducts(int count, {int seedOffset = 0}) {
  return List.generate(count, (i) {
    final n = i + seedOffset;
    final basePrice = 89.0 + (n % 6) * 30;
    return NVProduct(
      id: 'prod_$n',
      name: _mockNames[n % _mockNames.length],
      price: basePrice,
      compareAtPrice: n % 4 == 0 ? basePrice + 40 : null,
      imageUrl: _mockImageUrls[n % _mockImageUrls.length],
      sizes: const ['S', 'M', 'L', 'XL'],
      colors: [
        _swatchColors[n % _swatchColors.length],
        _swatchColors[(n + 1) % _swatchColors.length],
        _swatchColors[(n + 2) % _swatchColors.length],
      ],
      category: _mockCategories[n % _mockCategories.length],
      brand: _mockBrands[n % _mockBrands.length],
      rating: 3.5 + (n % 3) * 0.5,
      inStock: n % 7 != 0,
      isNew: n % 3 == 0,
    );
...
class _NewArrivalsScreenState extends State<NewArrivalsScreen> {
  ...
  Future<void> _loadInitial() async {
    ...
    // TODO(integration): replace with your real fetch (Firestore/API).
    await Future.delayed(const Duration(milliseconds: 900));
    if (!mounted) return;
    setState(() {
      _products = _mockProducts(12);
  ...
  Future<void> _loadMore() async {
    ...
    // TODO(integration): replace with real pagination (startAfter/page).
    await Future.delayed(const Duration(milliseconds: 800));
    if (!mounted) return;
    setState(() {
      final more = _mockProducts(8, seedOffset: _products.length);
      _products = [..._products, ...more];
      _isLoadingMore = false;
      if (_products.length >= 60) _hasMore = false;
    });
  }
  ...
  List<NVProduct> get _filteredProducts {
  ...
  void _onProductTap(NVProduct p) {}
  void _onAddToCart(NVProduct p) {}
  void _onToggleWishlist(NVProduct p) {}
  void _onQuickView(NVProduct p) {}
...
class _ProductArea extends StatelessWidget {
  ...
  final List<NVProduct> products;
  final SortOption sortOption;
  final ScrollController scrollController;
  final ValueChanged<SortOption> onSortChanged;
  final VoidCallback onFilterTap;
  final VoidCallback onResetFilters;
  final ValueChanged<NVProduct> onProductTap;
  final ValueChanged<NVProduct> onAddToCart;
  final ValueChanged<NVProduct> onToggleWishlist;
  final ValueChanged<NVProduct> onQuickView;
                    ...
                    children: [
                      ...
                      Positioned(
                        ...
                        child: AnimatedOpacity(
                          ...
                          child: IgnorePointer(
                            ignoring: !_hover,
                            child: _QuickViewButton(onTap: widget.onQuickView),
                          ),
                        ),
                      ),
                    ],
    ...
    return Column(
      ...
      children: [
        ...
        _FilterSection(
          title: 'Brand',
          child: Column(
            children: _mockBrands
                .map(
                  (b) => _CheckRow(
                    label: b,
                    checked: selectedBrands.contains(b),
                    onTap: () => onBrandToggled(b),
                  ),
                )
                .toList(),
          ),
